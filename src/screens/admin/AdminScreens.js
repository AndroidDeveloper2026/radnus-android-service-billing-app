...
export default function AdminScreens() {
  ...
  const handleDelete = useCallback(
      ...
      Alert.alert('Delete', `Delete "${name}"?`, [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            switch (type) {
              case 'make':
                deletedItem = makes.find(m => m.id === id);
                dispatch(removeMakeLocally(id));
                if (selectedMakeId === id) setSelectedMakeId('');
                break;
              case 'model':
                deletedItem = models.find(m => m.id === id);
                dispatch(removeModelLocally(id));
                break;
              case 'fault':
                deletedItem = faults.find(f => f.id === id);
                dispatch(removeFaultLocally(id));
                break;
              case 'drawer':
                deletedItem = drawers.find(d => d.id === id);
                dispatch(removeDrawerLocally(id));
                break;
              case 'engineer':
                deletedItem = engineers.find(e => e.id === id);
                dispatch(removeEngineerLocally(id));
                break;
              default:
                return;
            }
            ...
            try {
              ...
              switch (type) {
                case 'make':
                  result = await dispatch(deleteMake(id)).unwrap();
                  toast.show('Make deleted', { type: 'success' });
                  break;
                case 'model':
                  result = await dispatch(deleteModel(id)).unwrap();
                  toast.show('Model deleted', { type: 'success' });
                  break;
                case 'fault':
                  result = await dispatch(deleteFault(id)).unwrap();
                  toast.show('Fault deleted', { type: 'success' });
                  break;
                case 'drawer':
                  result = await dispatch(deleteDrawer(id)).unwrap();
                  toast.show('Drawer deleted', { type: 'success' });
                  break;
                case 'engineer':
                  result = await dispatch(deleteEngineer(id)).unwrap();
              ...
              // ❌ ROLLBACK: Restore the item if deletion failed
              if (deletedItem) {
                switch (type) {
                  case 'make':
                    dispatch(restoreMakeLocally(deletedItem));
                    break;
                  case 'model':
                    dispatch(restoreModelLocally(deletedItem));
                    break;
                  case 'fault':
                    dispatch(restoreFaultLocally(deletedItem));
                    break;
                  case 'drawer':
                    dispatch(restoreDrawerLocally(deletedItem));
                    break;
                  case 'engineer':
                    dispatch(restoreEngineerLocally(deletedItem));
                    break;
                }
              }
    ...
    [dispatch, selectedMakeId, toast, makes, models, faults, drawers, engineers],
  );
